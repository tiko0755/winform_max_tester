...
namespace nestCirelCal
{
    partial class UpdateFrm
    {
        ...
        public int InitUI_test(Form frmWindow, Control[] control, ref Work_Test workObj)
        {
            ...
            workObj.evnt_updateBtnStartBG += updateBtnStartBC;
            workObj.evnt_updateBtnStartTxt += updateBtnStartText;
            ...
            log("hello\",\" world");
            log("hello, china", Logger.LogType.Board);
        ...
        void updateTestTips(object sender, string tips, Color bc)
        {
            tBxTips.BeginInvoke(new EventHandler(delegate {
                tBxTips.Text = tips;
                tBxTips.BackColor = bc;
            }));
        }
        ...
        void updateTestLine(object sender, int lineIndx, string result, string judgement, double itemTime, Color itemColor) {
        ...
        void updateBtnStartBC(object sender, Color bc)
        {
            log("change backgroud color\",\" china\r\n", Logger.LogType.Board);
            btnStart.BeginInvoke(
                new EventHandler(delegate {
                    btnStart.BackColor = bc;
                })
            );
        }

        void updateBtnStartText(object sender, string txt)
        {
            btnStart.BeginInvoke(
                new EventHandler(delegate {
                    btnStart.Text = txt;
